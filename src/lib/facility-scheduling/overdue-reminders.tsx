...
import { formatMoney, toCents } from "@/lib/facility-scheduling/ar"
import { buildOverdueReminderEmail } from "@/lib/facility-scheduling/invoice-email"
import { InvoicePdf } from "@/lib/facility-scheduling/invoice-pdf"
import { loadInvoicePdfData } from "@/lib/facility-scheduling/invoice-pdf-data"
import { agingBucket, decideReminder } from "@/lib/facility-scheduling/overdue"
...
export async function runOverdueReminders(supabase: Client): Promise<ReminderSweepResult> {
  ...
  for (const settings of enabled) {
    ...
    if (invoicesError) {
      logServerError("facility-scheduling/overdue-reminders", invoicesError)
      result.failures += 1
      continue
    }
    ...
    for (const invoice of open) {
      ...
      try {
        ...
        if (!sentResult.ok) {
          logServerError("facility-scheduling/overdue-reminders", new Error(sentResult.error))
          result.failures += 1
          continue
        }
        ...
        logServerError("facility-scheduling/overdue-reminders", e)
        result.failures += 1
      }
    }
  }
