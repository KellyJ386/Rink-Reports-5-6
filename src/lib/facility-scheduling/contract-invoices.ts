...
import { deriveStatus, toCents } from "@/lib/facility-scheduling/ar"
import { deliverInvoiceEmail } from "@/lib/facility-scheduling/deliver-invoice-email"
import {
  generateInvoiceCore,
...
import {
  ...
  monthWindow,
  shouldComplete,
} from "@/lib/facility-scheduling/season-contracts"
    ...
    try {
      ...
      if (!generated.ok) {
        ...
        if ((reverted ?? []).length === 0) {
          logServerError(
            "facility-scheduling/contract-invoices",
            new Error(
              `LOST PERIOD ${decision.periodKey} on contract ${contract.id}: generation failed AND the cursor revert did not land`,
            ),
          )
        }
        logServerError("facility-scheduling/contract-invoices", new Error(generated.error))
        result.failures += 1
        continue
      }
      ...
      logServerError("facility-scheduling/contract-invoices", e)
      result.failures += 1
    }
  }
