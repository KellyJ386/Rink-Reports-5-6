...
import { sendDasherBoardsReport } from "../../../actions"
...
export function SendReportButton({ inspectionId }: { inspectionId: string }) {
  ...
  const handleSend = () => {
    ...
    startTransition(async () => {
      const result = await sendDasherBoardsReport(inspectionId)
      if (result.ok) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      {status.kind === "sent" && status.count === 0 ? (
        ...
        >
          No recipients are configured for dasher boards. Set up a send list in
