...
import { resolveResurfaceMinutes } from "@/lib/facility-scheduling/resurface"
import { planCutsForRinkDay, type PlannableBooking } from "@/lib/facility-scheduling/resurface-plan"
...
> {
  ...
  if (!(await currentUserCan(supabase, "rink_scheduling", "edit"))) {
    return { ok: false, error: "Planning cuts needs the Facility Scheduling edit permission." }
  }
...
export async function previewResurfacePlan(input: {
  ...
  try {
    ...
    logServerError("reports/facility-scheduling/resurface-plan-actions", e)
  ...
  try {
    ...
    if (!plan.cutTypeId) {
      return {
        ok: false,
        error:
          "No active Ice Resurface booking type is set up. Create one on the Lists tab of Facility Scheduling admin first.",
      }
    }
