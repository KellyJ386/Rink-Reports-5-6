...
import { resolveBufferMinutes } from "@/lib/facility-scheduling/buffer"
import { deliverBookingConfirmation } from "@/lib/facility-scheduling/deliver-booking-email"
import { countMatchedEntries } from "@/lib/facility-scheduling/waitlist-match"
...
const CALENDAR_PATH = "/reports/facility-scheduling"
...
function caught(e: unknown): { ok: false; error: string } {
  logServerError("reports/facility-scheduling/actions", e)
  ...
  try {
    ...
    if (input.status === "confirmed" && !canEdit) {
      return {
        ok: false,
        error:
          "You can create a tentative booking, but confirming it needs the Facility Scheduling edit permission.",
      }
    }
    ...
    if (emailOutcome.delivered === false && emailOutcome.reason === "failed") {
      logServerError(
        "reports/facility-scheduling/booking-confirmation",
...
export async function updateBooking(input: {
  ...
  try {
    ...
    if (!canEdit) {
      return {
        ok: false,
        error: "Editing a booking needs the Facility Scheduling edit permission.",
      }
    }
    if (new Date(input.endsAt).getTime() <= new Date(input.startsAt).getTime()) {
      return { ok: false, error: "The end time must be after the start time." }
...
export async function cancelBooking(
  ...
  try {
    ...
    if (!canEdit) {
      return {
        ok: false,
        error: "Cancelling a booking needs the Facility Scheduling edit permission.",
      }
    }
...
export async function quickCreateCustomer(
  ...
  try {
    ...
    if (!canEdit) {
      return {
        ok: false,
        error: "Adding a customer needs the Facility Scheduling edit permission.",
      }
    }
