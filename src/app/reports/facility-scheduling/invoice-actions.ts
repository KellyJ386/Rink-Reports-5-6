...
import {
  ...
  toCents,
  type InvoiceStatus,
} from "@/lib/facility-scheduling/ar"
import {
  generateInvoiceCore,
  listUninvoicedBookingsCore,
  type BillableBooking,
} from "@/lib/facility-scheduling/invoice-generation"
import { createClient } from "@/lib/supabase/server"
import { deliverInvoiceEmail, type InvoiceEmailOutcome } from "@/lib/facility-scheduling/deliver-invoice-email"
import { dayKeyInTz } from "@/lib/timezone"

import type { SimpleResult } from "./_lib/types"

const AR_PATH = "/reports/facility-scheduling/invoices"
...
> {
  ...
  if (!(await currentUserCan(supabase, "rink_scheduling", "edit"))) {
    return {
      ok: false,
      error: "Invoicing needs the Facility Scheduling edit permission.",
    }
  }
...
function caught(e: unknown): { ok: false; error: string } {
  logServerError("reports/facility-scheduling/invoice-actions", e)
  return { ok: false, error: e instanceof Error ? e.message : "Unknown error." }
}

// ---------------------------------------------------------------------------
// Uninvoiced bookings + generation — the logic lives in
// @/lib/facility-scheduling/invoice-generation so the season-contract cron
// generates from the SAME code path a biller uses. These actions only guard
// and delegate.
// ---------------------------------------------------------------------------

export type { BillableBooking } from "@/lib/facility-scheduling/invoice-generation"
