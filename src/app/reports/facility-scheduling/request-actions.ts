...
const REQUESTS_PATH = "/reports/facility-scheduling/requests"
...
> {
  ...
  if (!(await currentUserCan(supabase, "rink_scheduling", "edit"))) {
    return {
      ok: false,
      error: "This needs the Facility Scheduling edit permission.",
    }
  }
...
function caught(e: unknown): { ok: false; error: string } {
  logServerError("reports/facility-scheduling/request-actions", e)
