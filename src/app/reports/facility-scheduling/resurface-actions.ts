...
import { canTransition, type ResurfaceLifecycleStatus } from "@/lib/facility-scheduling/resurface"
...
> {
  ...
  if (!(await currentUserCan(supabase, "rink_scheduling", "edit"))) {
    return { ok: false, error: "Managing resurfaces needs the Facility Scheduling edit permission." }
  }
...
function caught(e: unknown): { ok: false; error: string } {
  logServerError("reports/facility-scheduling/resurface-actions", e)
