...
const CONTRACTS_PATH = "/reports/facility-scheduling/contracts"
...
> {
  ...
  if (!(await currentUserCan(supabase, "rink_scheduling", "edit"))) {
    return { ok: false, error: "Season contracts need the Facility Scheduling edit permission." }
  }
...
function caught(e: unknown): { ok: false; error: string } {
  logServerError("reports/facility-scheduling/contract-actions", e)
