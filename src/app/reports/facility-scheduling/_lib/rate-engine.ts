// Pure rate resolution for Facility Scheduling & Billing.
//
...
export function quoteBooking(input: QuoteInput): RateQuote {
  ...
  if (!card) {
    return {
      ...
      totalHours,
      problem:
        "No rate card covers this date, so the booking cannot be priced. Add one in Facility Scheduling admin.",
    }
  }
