...
import { CalendarClient } from "@/app/reports/facility-scheduling/_components/calendar-client"
import {
  loadCalendarData,
  type CalendarSearchParams,
} from "@/app/reports/facility-scheduling/_lib/load-calendar"
 ...
 * calendar at /reports/facility-scheduling is a read-only view by design.
 *
...
export default async function AdminSchedulePage({
  ...
  const nav = (
    <TabNav
      ariaLabel="Facility scheduling sections"
  ...
  if (!canView && !canCreate && !canEdit) {
    return (
      <div className="flex flex-col gap-6 p-4 md:p-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Facility Scheduling permission needed</CardTitle>
            <CardDescription>
              This account holds the admin console but no Facility Scheduling
  ...
  return (
    <PageHeader
      ...
      actions={
        <>
          <Link
            href="/reports/facility-scheduling/desk"
          ...
          {canEdit && (
            <>
              <Link
                href="/reports/facility-scheduling/invoices"
                className="text-muted-foreground text-sm no-underline hover:underline print:hidden"
              >
                Invoices →
              </Link>
              <Link
                href="/reports/facility-scheduling/insights"
                className="text-muted-foreground text-sm no-underline hover:underline print:hidden"
              >
                Insights →
              </Link>
              <Link
                href="/reports/facility-scheduling/requests"
                className="text-muted-foreground text-sm no-underline hover:underline print:hidden"
              >
                Requests →
              </Link>
              <Link
                href="/reports/facility-scheduling/contracts"
                className="text-muted-foreground text-sm no-underline hover:underline print:hidden"
              >
                Contracts →
              </Link>
