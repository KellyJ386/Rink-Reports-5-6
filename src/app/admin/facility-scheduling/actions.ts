...
const ADMIN_PATH = "/admin/facility-scheduling"
...
> {
  ...
  if (!allowed) {
    return {
      ok: false,
      error:
        "Your account has admin console access but not the Facility Scheduling edit permission. Ask a super admin to grant it in Permissions.",
    }
  }
...
> {
  ...
  if (!allowed) {
    return {
      ok: false,
      error:
        "Rate cards and module settings need the Facility Scheduling admin permission. Ask a super admin to grant it in Permissions.",
    }
  }
  return { ok: true, facilityId: facility.facilityId }
}
...
function caught(e: unknown): ActionState {
  logServerError("admin/facility-scheduling/actions", e)
 ...
 */
export async function seedFacilitySchedulingDefaults(): Promise<SimpleResult> {
  try {
    const facility = await ensureFacilityManager()
    if (!facility.ok) return { ok: false, error: facility.error }
