...
**The idea.** Generate a yearly export file per facility (PDF/CSV), hand it to the facility
representative to hold for the 7-year window, then purge those years from the DB. Goal:
minimize stored data.

**Rejected as the *system of record*** — it diminishes precisely the evidentiary value the
7-year retention exists to protect:

1. **Chain of custody inverts.** Today the operator controls the record (RLS + audit
   triggers, migrations 41/46/93). Handing the only copy to the *subject* of the record makes
   them its custodian. For incident/accident reports especially, "the facility can't find
   their 2026 file" is not a defensible answer to a year-6 injury claim or subpoena.
   Liability / insurance / regulatory regimes generally expect the operator to **retain and
   produce**, not delegate.
2. **Tamper evidence evaporates.** An `audit_logs` row with trigger-enforced history is far
   more defensible than an emailed PDF a rep can lose, edit, or re-create.
3. **Queryability dies.** Structured rows → flat file means no cross-facility trend analysis,
   threshold-breach search, refrigeration anomaly detection, or ice-depth / air-quality
   severity history for the archived years.
4. **Availability becomes N×fragile.** One backed-up DB vs. 1,000 reps each reliably keeping
   a file for 7 years — the latter's fleet-wide failure rate is effectively 100%.

**Premise check.** Structured report rows are tiny (audit_logs ≈ 242 rows today); 7 years
across 1,000 facilities is plausibly low-tens of GB — cents of Postgres storage. The real
pain is purge bloat and scan cost, which partitioning already fixes more cheaply. Measure
before optimizing; the heavy offender is `ice_depth_measurements` (per-session fan-out), not
the report tables.

**The version that works (keep custody, flip the offload):**

- **Tiered cold-storage archive, not offload.** Instead of `DROP`ping expired partitions,
  `DETACH` and dump them to operator-owned cold object storage (Supabase Storage / S3),
  compressed. Cheap, still ours, still RLS-gated on retrieval. Shrinks the hot DB without
  surrendering the record. (Folds naturally into rollout step 4.)
- **Yearly file as a value-add, not the backstop.** Generating a yearly PDF/CSV per facility
  for *their* convenience is fine — give it to reps freely, but keep the authoritative copy.
  Same generation logic, opposite conclusion about who is the system of record.
