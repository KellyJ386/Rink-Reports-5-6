...
- **Phase 6 shipped** (D9, per the §6 rewrite — no Dexie): per-user raw-IndexedDB read model `rink-daily-areas-cache` (`src/lib/offline/daily-areas-cache.ts`, mirroring the schedule-cache pattern) seeded by every online render of the landing view, self-invalidating by facility-local business date (not TTL); data-free `/offline-daily` shell + `OfflineMyAreas` view, cached by the SW alongside `/offline-schedule` (cache bumped to v6, fallback page links both shells); cache wiped on sign-out/user-switch; assignment mutations disabled offline with an inline notice (no queueing); and the stale-assignment sync-rejection UX — a replay rejected by the assignment/access RLS now parks immediately as a permanent 422 with copy explaining the assignment changed, surfaced on the Pending Sync Queue page. Entitlement caveat verified: the cache only ever stores the server's RLS-scoped response for the signed-in user.
