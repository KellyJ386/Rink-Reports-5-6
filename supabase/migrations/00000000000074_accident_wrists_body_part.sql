-- -----------------------------------------------------------------------------
-- Add 'wrists' as a body_part dropdown for accident reports.
--
-- The staff submission diagram now renders a wrist zone between forearm and
-- hand. The form will only persist a wrist selection if a matching dropdown
-- row exists, so seed it for every existing facility and update the
-- seed_default_accident_dropdowns function so new facilities get it too.
-- -----------------------------------------------------------------------------

insert into public.accident_dropdowns
  (facility_id, category, key, display_name, sort_order, is_active)
select f.id, 'body_part', 'wrists', 'Wrists', 17, true
...
begin
  ...
  -- canonical zones going forward. wrists sit between elbows and hands.
  insert into public.accident_dropdowns
    (facility_id, category, key, display_name, sort_order, is_active)
  values
