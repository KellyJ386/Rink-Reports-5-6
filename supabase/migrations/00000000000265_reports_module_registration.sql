...
comment on function public.seed_default_facility_modules(uuid) is
  'Seeds facility_modules with every canonical module enabled (incl. reports as of migration 265). Idempotent via on conflict do nothing on (facility_id, module_key).';
...
comment on function public.canonical_role_permission_grants() is
  'Canonical per-role default permission grants (expanded to cumulative actions), keyed by role key. Source for seed_role_permission_defaults_for_facility() and the roles auto-seed trigger. reports added in migration 265.';
...
begin
  ...
  if v_lost > 0 then
    raise exception
      'migration 265 DROPPED % pre-existing grant(s) from canonical_role_permission_grants() (e.g. %). '
      'Restate the WHOLE matrix — migration 198 existed only to repair migration 193 making exactly this mistake.',
      v_lost, v_sample;
  end if;

  raise notice 'migration 265: canonical grant matrix is purely additive (% grants before, % after)',
...
begin
  ...
  if v_config not like '%search_path=public, pg_temp%' then
    raise exception
      'migration 265: canonical_role_permission_grants() lost its pinned search_path (now %). '
  ...
  if v_vol <> 'i' then
    raise exception
      'migration 265: canonical_role_permission_grants() is no longer IMMUTABLE (volatility %)', v_vol;
  end if;

  raise notice 'migration 265: function attributes preserved (immutable, %)', v_config;
end $$;
...
declare
  r record;
begin
  raise notice 'migration 265: facility_modules rows for reports: %',
    (select count(*) from public.facility_modules where module_key = 'reports');
  raise notice 'migration 265: role_permission_defaults rows for reports: %',
  ...
  loop
    raise notice 'migration 265: reports granted to % user(s) with role %', r.users, r.role_key;
  end loop;
end $$;
