-- =============================================================================
-- 00000000000106_seed_daily_report_checklists.sql
-- Daily Reports: seed the standard Operations Checklists catalog.
--
-- Source: RinkReports 5-6 Operations Checklists workbook. 17 categories, each
-- with three phase templates (Opening / Operational / Closing) and their
-- checklist items (506 items total).
--
-- Facility-scoped to Tennity by design (mirrors migration 80): the hardcoded
-- facility id only matches in the production project, so this is a no-op on
-- fresh/local databases (no matching facility => zero inserts).
--
-- Idempotent: areas upsert on (facility_id, slug) DO NOTHING; templates and
-- items insert only WHERE NOT EXISTS. Re-running never duplicates rows and
-- never clobbers admin renames/reorders/deactivations.
-- =============================================================================
...
join public.daily_report_areas a
  on a.facility_id = '4490bad7-ef1b-4544-8d7f-7aea49884550'::uuid and a.slug = t.area_slug
where not exists (
...
from item i
join public.daily_report_areas a on a.facility_id = '4490bad7-ef1b-4544-8d7f-7aea49884550'::uuid and a.slug = i.area_slug
