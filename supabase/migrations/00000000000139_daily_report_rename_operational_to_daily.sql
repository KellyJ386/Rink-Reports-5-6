...
--
-- Product change: the middle daily-report shift phase is renamed from
-- "Operational" to "Daily", so each area's shift set reads Opening / Daily /
-- Closing. This is a rename (not a delete) — the phase keeps all of its
-- existing checklist items; only the template name changes.
--
-- 1. Backfill: rename every existing 'Operational' template to 'Daily' across
--    ALL facilities. Collision-safe: an area that somehow already has a 'Daily'
--    template is skipped so we never create a duplicate phase.
-- 2. Re-create seed_default_daily_report_checklists so newly created facilities
--    seed 'Daily' instead of 'Operational'. The body is identical to migration
--    135 except the phase name; still idempotent, still service_role /
--    definer-internal only.
--
-- Per-facility counts are unchanged (51 phase templates / 506 items), so the
-- SEED-135 assertions in supabase/tests/rls_isolation.sql still hold. Existing
-- daily_report_submissions are unaffected (they reference template_id and
-- snapshot their own item labels).
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. Rename existing 'Operational' phase templates to 'Daily' (all facilities).
--    sort_order is unchanged, so ordering stays Opening (0) / Daily (1) /
--    Closing (2). Skip any area that already has a 'Daily' template.
-- -----------------------------------------------------------------------------
update public.daily_report_templates dt
   set name = 'Daily'
 where dt.name = 'Operational'
   and not exists (
     select 1
       from public.daily_report_templates d2
      where d2.area_id = dt.area_id
        and d2.name = 'Daily'
   );

-- -----------------------------------------------------------------------------
-- 2. Re-seed function: same catalog as migration 135 with the renamed phase.
-- -----------------------------------------------------------------------------
