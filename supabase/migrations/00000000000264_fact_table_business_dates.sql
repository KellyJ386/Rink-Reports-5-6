...
comment on function public.stamp_business_date_from() is
  ...
  'migration 264 for the seven module fact tables.';
...
begin
  for r in
    ...
    order by 1
  loop
    raise notice 'migration 264: business_date backfilled for % row(s) in %', r.n, r.t;
    v_total := v_total + r.n;
  end loop;
  raise notice 'migration 264: % row(s) backfilled across 7 fact tables', v_total;
end $$;
...
comment on column public.ice_depth_sessions.business_date is
  ...
  'facilities.timezone and stamped server-side (migration 264). The reporting '
...
comment on column public.refrigeration_reports.business_date is
  ...
  'facilities.timezone and stamped server-side (migration 264).';
...
comment on column public.dasher_boards_inspections.business_date is
  ...
  'facilities.timezone and stamped server-side (migration 264). An OPEN walk is '
