...
2. Inside server components / route handlers, **always go through `src/lib/auth`** rather than calling Supabase directly:
   ...
   - `requireUser()` / `requireAdmin()` / `requireModuleAdmin(moduleName)` — server-only guards that `redirect("/login")` or `redirect("/forbidden")`. `requireModuleAdmin` checks the *module-scoped* `admin` grant (same source of truth as the RLS `has_module_admin_access` helper); module consoles whose writes are RLS-gated per-module (e.g. `/admin/communications`) call it in addition to `requireAdmin`, because the global admin gate does not imply the module-scoped one. `requireAdmin` allows `users.is_super_admin = true`, OR (primary, permission-model check) an enabled `user_permissions` row for `module_name = 'admin'` / `action = 'admin'` in the user's `facility_id`, OR (fallback for accounts not yet backfilled into `user_permissions`) an active `employees` row whose `role.key in (admin, super_admin)`. The role set is the **live** model: roles are `super_admin / admin / manager / staff` (plus per-facility custom roles such as `driver`) — `gm`/`supervisor` were retired (migrations 58/87; `gm` folded into `admin`). Authorization is resolved through `user_permissions`, not a fixed role tier; roles only seed permission defaults.
...
The model is **facility-local everywhere**: users enter wall-clock times in the facility's IANA zone (`facilities.timezone`), the DB stores real UTC instants, and `src/lib/timezone.ts` converts. Two rules follow:

- **Never format a time without an explicit `timeZone`.** `Intl.DateTimeFormat` defaults to whatever zone the code runs in — the *server's* (UTC in production) for server components, server actions, cron routes, and email bodies. That silently misstates every time and can name the wrong day. `formatShiftWindow` / `formatDateOnly` / `formatDateTime` / `formatTimeRange` all take a zone; pass the facility's.
- **The admin scheduling grid runs on facility-zoned pseudo-local Dates.** `dtoToEvent` (`admin/scheduling/shifts/_lib/board-model.ts`) projects stored instants through `utcToZonedDate`, so every local getter in the board (`getHours`, `getDay`, `setHours`, drag math, CSV export, "save as template") reads the *rink's* wall clock. Their epoch values are NOT the stored instants — the board converts back via `zonedDateToUtc` at its single persist boundary (`toUtcIso` in `week-board.tsx`). Add new server payloads through that helper, never `date.toISOString()`.

Facility operating hours live on `schedule_settings.operating_hours_{start,end}_minute` (minutes since local midnight; `1440` = midnight) as of migration 232, read via `resolveOperatingHours`. The legacy `facilities.settings → scheduling.operating_hours` jsonb is still read as a fallback but can't be written in-app (`facilities` is super-admin-write-only).
