...
/**
 * Facility Scheduling & Billing (Module 12) — the most money-critical module in
 ...
 *     writes live on the admin surface at /admin/facility-scheduling/schedule,
 *     so no edit affordance may ever render on /reports/facility-scheduling;
...
const CALENDAR = "/reports/facility-scheduling"
const ADMIN_SCHEDULE = "/admin/facility-scheduling/schedule"

/** Opens a facility-scheduling page; false when the module is gated off for the
 *  account (the NotAvailable card) or the account lacks a facility. */
async function openFacilityScheduling(
  page: import("@playwright/test").Page,
  path = "",
): Promise<boolean> {
  await page.goto(`${CALENDAR}${path}`)
  const denied = page.getByText(/facility schedule not available/i)
...
test.describe("15. Facility Scheduling (staff)", () => {
  ...
  test("calendar renders with all four views reachable", async ({ page }) => {
    const opened = await openFacilityScheduling(page)
    test.skip(!opened, "TODO(seed): rink_scheduling not enabled for this account")

    await expect(page.getByRole("heading", { name: /facility schedule/i })).toBeVisible()
  ...
  test("edit-tier affordances gate together, not piecemeal", async ({ page }) => {
    ...
    const deniedInvoices = await page
      .getByText(/facility schedule not available/i)
      .isVisible()
      .catch(() => false)
  ...
  test("money pages deny a non-edit account by rendering the gate, not data", async ({
    ...
    for (const path of ["/invoices", "/insights", "/requests", "/contracts"]) {
      await page.goto(`${CALENDAR}${path}`)
      await expect(page.getByText(/facility schedule not available/i)).toBeVisible()
    }
  })

  test("front desk view is view-tier and renders", async ({ page }) => {
    const opened = await openFacilityScheduling(page, "/desk")
    test.skip(!opened, "TODO(seed): rink_scheduling not enabled for this account")
  ...
  test("agenda view offers the print button", async ({ page }) => {
    const opened = await openFacilityScheduling(page, "?view=list")
    test.skip(!opened, "TODO(seed): rink_scheduling not enabled for this account")
...
test.describe("15. Facility Scheduling (admin)", () => {
  test.beforeEach(async ({ page }) => {
  ...
  async function openAdminSchedule(
    ...
    const gated = page.getByText(/facility scheduling permission needed/i)
  ...
  test("admin console tabs all render", async ({ page }) => {
    for (const tab of ["setup", "rates", "lists", "displays", "settings"]) {
      await page.goto(`/admin/facility-scheduling?tab=${tab}`)
      await expect(page).not.toHaveURL(/\/forbidden|\/login/)
      await expect(page.locator("body")).not.toContainText(
        /application error|something went wrong/i,
      )
